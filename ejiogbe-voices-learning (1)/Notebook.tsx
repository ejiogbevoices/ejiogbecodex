...
import { Search, Trash2, Bookmark, Mic, Languages, Calendar, AlertCircle, X, CheckCircle2, Plus, Target, BarChart3, Clock, Trophy } from 'lucide-react';
...
const Notebook: React.FC = () => {
  ...
  const totalXP = sessions.reduce((acc, curr) => acc + (curr.xpGained || 0), 0);
      ...
      {activeTab === 'progress' && (
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  <div className="bg-slate-800/50 p-6 rounded-2xl border border-slate-700 flex flex-col justify-between">
                      <div className="flex items-center gap-3 text-slate-400 mb-2">
                          ...
                          <span className="text-xs font-bold uppercase tracking-wider">Total Experience</span>
                      </div>
                      <div className="text-3xl font-serif text-slate-100 font-bold">
                          {totalXP.toLocaleString()} <span className="text-lg text-slate-500 font-sans">XP</span>
                      </div>
                  </div>
                  <div className="bg-slate-800/50 p-6 rounded-2xl border border-slate-700 flex flex-col justify-between">
                      <div className="flex items-center gap-3 text-slate-400 mb-2">
              ...
              <div className="bg-slate-900/30 rounded-3xl border border-slate-800 p-6">
                  <h3 className="text-lg font-bold text-slate-300 mb-4">Recent Activity</h3>
                              ...
                              <div key={session.id} className="flex flex-col sm:flex-row sm:items-center justify-between p-4 bg-slate-800/30 rounded-xl border border-slate-800/50">
                                  ...
                                  <div className="mt-2 sm:mt-0 flex items-center gap-4">
                                      <div className="text-right">
                                          ...
                                          <div className="font-mono font-bold text-indigo-400">+{session.xpGained || 0}</div>
                                      </div>
                                      <div className="text-right">
